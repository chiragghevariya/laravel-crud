...
<div class="container">
  <form action="{{route('area.save-edit-form')}}" method="POST">
    @csrf
    ...
    <select id="City" name="city">
      ...
      <option value="{{$s->id}}" @if($editdata->city_name == $s->id) selected @endif>{{$s->city_name}}</option>
    ...
    <input type="text" id="area" value="{{$editdata->area_name}}" name="area" placeholder="Your area name">

    <label>status</label>
        <input type="radio" value="1" name="status"  @if($editdata->status == 1) checked @endif>active
        <input type="radio" value="2" name="status"  @if($editdata->status == 2) checked @endif>inactive
